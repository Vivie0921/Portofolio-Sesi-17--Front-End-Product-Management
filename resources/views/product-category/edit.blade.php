<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto mt-10">
        <div class="bg-white p-6 rounded-lg shadow-md">
            ...
            <form action="{{ route('product-category.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                <!-- Nama Kategori Produk -->
                <div>
                    <label for="nama" class="block text-sm font-medium text-gray-700">Nama Kategori Produk</label>
                    <input type="text" id="nama" name="nama" value="{{ $category->name }}" required
