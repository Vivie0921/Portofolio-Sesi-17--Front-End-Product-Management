        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            ...
            <form action="{{ route('product.update',$product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="number" id="stok" name="stok" min="0" value="{{ $product->stock }}"required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="harga" name="harga" min="0" step="0.01" value="{{ $product->price }}"required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <input type="file" id="gambar" name="gambar" accept="image/*" value="{{ $product->image }}"required
                        class="mt-1 block w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4
                        file:rounded-md file:border-0 file:text-sm file:font-semibold
                        file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                        @if ($product->image)
                            <div class="mt-2">
                                <img src="{{ $product->image }}" alt="{{ $product->name }}" class="w-40 h-40 object-cover rounded">
                            </div>
                        @endif
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori" value="{{ $product->product_category_id }}"required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
